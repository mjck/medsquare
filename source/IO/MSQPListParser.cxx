...
QVariant MSQPListParser::parsePList(QIODevice *device) {
	QVariantMap result;
	QDomDocument doc;
	...
	if (!success) {
		qDebug() << "MSQPListParser Warning: Could not parse PList file!";
	...
	//	qDebug() << "MSQPListParser Warning: plist is using an unknown format version, parsing might fail unexpectedly";
	//}
...
QVariant MSQPListParser::parseElement(const QDomElement &e) {
	...
	if (tagName == QLatin1String("dict")) {
		...
		qDebug() << "MSQPListParser Warning: Invalid tag found: " << e.tagName() << e.text();
	}
	return result;
}

QVariantList MSQPListParser::parseArrayElement(const QDomElement& element) {
	QVariantList result;
...
QVariantMap MSQPListParser::parseDictElement(const QDomElement& element) {
	QVariantMap result;
	QDomNodeList children = element.childNodes();
	QString currentKey;
	for (int i = 0; i < children.count(); i++) {
