/*=========================================================================
 ...
 Program:   MedSquare
 Module:    vtkmsqGDCMMosaicImageReader.cxx
 ...
 * 
 */
static void vtkmsqGDCMMoisacImageReader_reorganize_mosaic(const unsigned short *input,
    const unsigned int *inputdims, unsigned int square, const unsigned int *outputdims,
 ...
 * 
 */
inline bool vtkmsqGDCMMoisacImageReader_IsCharTypeSigned()
{
 ...
 * 
 */
vtkmsqGDCMMoisacImageReader::vtkmsqGDCMMoisacImageReader()
{
 ...
 * 
 */
int vtkmsqGDCMMoisacImageReader::CanReadFile(const char* fname)
{
  gdcm::ImageReader reader;
 ...
 * 
 */
int vtkmsqGDCMMoisacImageReader::ProcessRequest(vtkInformation* request,
 ...
 * 
 */
bool vtkmsqGDCMMoisacImageReader::ComputeMOSAICDimensions(gdcm::File &file,
    unsigned int dims[3])
{
  gdcm::CSAHeader csa;
  ...
  std::vector<unsigned int> colrow = gdcm::ImageHelper::GetDimensionsValue(file);
  dims[0] = colrow[0];
  dims[1] = colrow[1];
 ...
 * 
 */
void vtkmsqGDCMMoisacImageReader::FillMedicalImageInformation(
 ...
 * 
 */
int vtkmsqGDCMMoisacImageReader::RequestInformationCompat()
{
  ...
  {
    // hey! I need at least one file to chew on !
  ...
  const unsigned int *inputdims = image.GetDimensions();
  //printf("input dimensions=%d %d %d\n", inputdims[0], inputdims[1], inputdims[2]);

  // real dataset dimensions
  unsigned int dims[3] = { 0, 0, 0 };

  // check whether the file is moisac (i.e. tiled)
  bool isMosaic = ComputeMOSAICDimensions(file, dims);

  if (!isMosaic)
  {
    // if not, assume original dimensions
    dims[0] = inputdims[0];
    dims[1] = inputdims[1];
    dims[2] = inputdims[2];
  }
  ...
  if (image.GetNumberOfDimensions() == 2)
  {
    ...
    // of the last axis (see vtkImageReader2::SetFileNames )

    // multiple slices
    if (this->FileNames && this->FileNames->GetNumberOfValues() > 1)
    {
      this->DataExtent[4] = 0;
      this->DataExtent[5] = dims[2] * this->FileNames->GetNumberOfValues() - 1; // for moisac, multiply it by dims[2]
    }
    else
    {
      // even a single slice may result in a 3D image
      this->DataExtent[4] = 0;
...
#else
      if( !vtkmsqGDCMMoisacImageReader_IsCharTypeSigned() )
      {
 ...
 * 
 */
int vtkmsqGDCMMoisacImageReader::RequestInformation(vtkInformation *request,
    vtkInformationVector **inputVector, vtkInformationVector *outputVector)
{
  (void) request;
  (void) inputVector;
 ...
 * 
 */
int vtkmsqGDCMMoisacImageReader::LoadSingleFile(const char *filename, char *pointer,
    unsigned long &outlen)
{
  ...
  if (isMosaic)
    outlen = data->GetScalarSize() * dims[0] * dims[1] * dims[2];
  else
    outlen = len;
  ...
  else
  {
    ...
    if (isMosaic)
    {
      ...
      vtkmsqGDCMMoisacImageReader_reorganize_mosaic((unsigned short*) &buf[0],
          image.GetDimensions(), div, dims, (unsigned short*) pointer);
    }
    else
 ...
 * 
 */
int vtkmsqGDCMMoisacImageReader::RequestDataCompat()
{
  ...
  else if (this->FileNames && this->FileNames->GetNumberOfValues() >= 1)
  {
    ...
    unsigned long len = 0;
    for (int j = 0; j < this->FileNames->GetNumberOfValues(); j++)
    {
      ...
      pointer += len;
      this->UpdateProgress((double) j / (this->FileNames->GetNumberOfValues() - 1));
    }
  }
  else
  {
  ...
  if (!this->FileLowerLeft)
  {
    vtkmsqGDCMMoisacImageReader_InPlaceYFlipImage(this->GetOutput(0));
  }
 ...
 * 
 */
int vtkmsqGDCMMoisacImageReader::RequestData(vtkInformation *vtkNotUsed(request),
 ...
 * 
 */
void vtkmsqGDCMMoisacImageReader::PrintSelf(ostream& os, vtkIndent indent)
{
