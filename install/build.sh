...
download_qt()
{
    ...
    ls -lh qt-everywhere-src-*.tar.gz
    if [ -e qt-everywhere-opensource-src-$qt_version.tar.gz ]; then
	md5check=`md5sum qt-everywhere-opensource-src-$qt_version.tar.gz`
	if [ "$md5check" == "7960ba8e18ca31f0c6e4895a312f92ff qt-everywhere-opensource-src-4.8.1.tar.gz" ]; then
	    return
	fi
	rem qt-everywhere-opensource-src-4.8.1.tar.gz
    fi	
    curl -O -C - "http://releases.qt-project.org/qt4/source/qt-everywhere-opensource-src-$qt_version.tar.gz"
    echo "ok?"
    read
}
download_vtk()
...
sanity_tests()
{
    ...
    # qt compilation requires the package libxext-dev on debian (how to test this on all distros?)
    # test for libXt-devel
    # test for swig
    # test for tk
    # test for uuid
    echo " Ok."
...
qt_install()
{
    ...
    # this function will take care of not downloading when appropriate
    download_qt

    # unpacking
    if [ ! -e qt-everywhere-opensource-src-$qt_version ]; then
        tar -xzvf qt-everywhere-opensource-src-$qt_version.tar.gz
    ...
    #QMAKE_INCDIR_X11=/usr/include/X11 QMAKE_LIBDIR_X11=/usr/lib/X11 ./configure --prefix=$install_prefix -opensource -confirm-license -release -no-webkit -no-javascript-jit -nomake demos -nomake examples -nomake docs -nomake translations -nomake webkit -no-script -no-scripttools -continue -nomake tools
    QMAKE_INCDIR_X11=/usr/include/X11 QMAKE_LIBDIR_X11=/usr/lib/X11 ./configure --prefix=$install_prefix -opensource -confirm-license -release -no-webkit -no-javascript-jit -nomake demos -nomake examples -nomake docs -nomake translations -nomake webkit -no-script -no-scripttools -continue -nomake tools -no-phonon -no-qt3support -no-accessibility -fast -no-dbus
    if [ ! $? -eq 0 ]; then
        echo "ERROR in configuring Qt."
        exit 1
    fi
    run_and_test "make"
    run_and_test "make install"
    set_version qt $qt_version
...
# try to detect distribution, check for missing packages and break if so.
if [ -e /etc/issue ]; then
    ubuntu=`grep -i ubuntu /etc/issue`
    debian=`grep -i debian /etc/issue`
    fedora=`grep -i fedora /etc/issue`
fi

if [ -n "$ubuntu" ]; then
    failure=0
    packages="c++ libxrender-dev python dev libxt-dev libglu1-mesa-dev libosmesa6-dev"
    for package in $packages; do
	pkg_test=`dpkg --get-selections | grep $package | grep install`
	if [ -z "$pkg_test" ]; then
	    echo "Missing $package! Please run 'sudo apt-get install $package'."
	    failure=1
	fi
    done
...
if [ -n "$debian" ]; then
    failure=0
    packages="g++ libxi-dev libxft-dev libxrandr-dev libxrender-dev libxfixes-dev libxext-dev libxcursor-dev libx11-dev libfreetype6-dev libfontconfig1-dev make"
    for package in $packages; do
	pkg_test=`dpkg --get-selections | grep $package | grep install`
	if [ -z "$pkg_test" ]; then
	    echo "Missing $package! Please run 'sudo apt-get install $package'."
...
if [ -n "$fedora" ]; then
    echo "Using build_old.sh."
    shift
    ./install/build_old.sh $*
    exit $?
fi

# unknown OS
if [ -z "$debian" -a -z "$ubuntu" -a -z "$fedora" ]; then
    echo "Using build_old.sh."
    shift
    ./install/build_old.sh $*
    exit $?
...
if [ -e $install_prefix/build.sh ]; then
    difference=`diff $0 $install_prefix/build.sh`
    if [ ! "${difference}1" == "1" ]; then
	echo "Current build script different from last used. Forcing removal of binaries."
	keep_builds="false"
	rm $versions_file
    fi
else
    echo "Previous build script missing. Forcing removal of binaries."
    keep_builds="false"
    rm $versions_file
fi
cp $0 $install_prefix/build.sh

if [ "$keep_builds" == "false" ]; then
    echo "####   Removing object files   ####"
    remd cmake-$cmake_version
    remd qt-everywhere-opensource-src-$qt_version
    remd vtk_build
    remd vtk.build
    remd VTK
...
if [ "$keep_sources" == "false" ]; then
    ...
    rem qt-everywhere-opensource-src-$qt_version.tar.gz
